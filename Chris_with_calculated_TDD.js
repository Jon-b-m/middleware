function middleware(iob, currenttemp, glucose, profile, autosens, meal, reservoir, clock, pumphistory) {
    ...
    // This middleware only works if you have added pumphistory to middleware in FreeAPS X code (my pumphistory_mw branch).
    ...
    // Turn off Chris' formula (and AutoISF) when using a temp target >= 117 (6.5 mol/l) and if an exercise setting is enabled.
    // If using AutoISF uncomment the profile.use_autoisf = false
    if (currentMinTarget >= 117 && exerciseSetting == true) {
    ...
    } else { return "Chris' formula is currently off." }
}
